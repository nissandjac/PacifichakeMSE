...
for(int time=0;time<tEnd;time++){ // Start time loop
  ...
  Z(i) = Freal(i)+Myear(i);
  Surveyobs(time) += surveyselc(i)*wage_survey(i,time)*Ntmp(i)*exp(-0.*Myear(i))*q;
  Ntot_survey += surveyselc(i)*Ntmp(i)*exp(-0.*Myear(i)); // To use with age comps
  ...
  if(flag_survey(time) == 1){ // Flag if  there was a measurement that year
    ...
    for(int i=0;i<(nage-1);i++){ // Loop over other ages
      if(i < age_maxage){
      age_survey_est(i,time) = ((surveyselc(i+1)*Ntmp(i+1)*exp(-0.*Myear(i)))/Ntot_survey);
      }else{
      age_survey_est(age_maxage-1,time) += ((surveyselc(i+1)*Ntmp(i+1)*exp(-0.*Myear(i)))/Ntot_survey);
      }
    }
  }  //Recruitment
