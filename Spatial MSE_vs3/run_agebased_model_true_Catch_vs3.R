...
run.agebased.true.catch <- function(df, seed = 100){
  ...
  for (yr in 1:(nyear)){ # Loop over years add one year for initial distribution
    ...
    for (season in 1:nseason){
      for (space in 1:nspace){
        ...
        if(season == 1){
          ...
          N.tmp <- N.save.age[,idx-1,space,season]*exp(-Mseason*pope.mul)#
          
          if(season == df$surveyseason){
            N.survey <- rep(0,nage)  
            N.survey <- N.save.age[,idx-1,space,season]*exp(-Mseason*0.5)# Halfway through whatever season
          }
          
        }else{
          B.tmp <-  sum(N.save.age[,idx,space,season-1]*exp(-Mseason*pope.mul)*w_catch*Fsel) # Get biomass from previous season
          N.tmp <- N.save.age[,idx,space,season-1]*exp(-Mseason*pope.mul)
          
          if(season == df$surveyseason){
            N.survey <- rep(0,nage)
            N.survey <- N.save.age[,idx,space,season-1]*exp(-Mseason*0.5)
          }
        }
        
        Fnew <- calcF(method = 'hybrid', E.temp = E.temp,B.tmp ,weight = w_catch,N = N.tmp,M = Mseason, Fsel)
        Fseason <- Fnew*Fsel
        # Fnew <- df$F0[yr]*Fnseason[season]*Fspace[space]
        ...
        if (season == 1){
          ...
          Catch.save.age[, idx,space, season] <- (Fseason/(Z))*(1-exp(-(Z)))*N.tmp*w_catch#N.save.age[, idx,space,season]*w_catch
          CatchN.save.age[, idx,space, season] <- (Fseason/(Z))*(1-exp(-(Z)))*N.tmp#N.save.age[, idx,space,season]
          ...
          N.C <- N.save.age[,idx,space,season]
          SSB.all[idx,season,space]<- sum(N.save.age[,idx,space,season]*Mat.sel, na.rm = T)*0.5
          Catch.save.age[, idx,space, season] <- (Fseason/(Z))*(1-exp(-(Z)))*N.C*w_catch
          CatchN.save.age[, idx,space, season] <- (Fseason/(Z))*(1-exp(-(Z)))*N.C
        }
    ...
    for (space in 1:nspace){
      survey.true[space,idx] <- sum(N.survey*surv.sel*q*w_surv)
