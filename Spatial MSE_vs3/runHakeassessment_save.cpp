...
Ninit(0) = Rinit;
for(int i=1;i<(nage-1);i++){
    Ninit(i) = Rinit * exp(-(Meq(i-1)))*exp(-0.5*0*SDR*SDR+initN(i-1));
  }
//
Ninit(nage-1) = Rinit*exp(-(Meq(nage-2)))/(Type(1.0)-exp(-M(nage-1)))*exp(-0.5*0*SDR*SDR+initN(nage-2));

  for(int i=0;i<(nage-1);i++){ // Loop over other ages
    SSBinit(nage-1) += Ninit(i)*Matsel(i)*0.5;
}
...
for(int time=0;time<(nage-1);time++){ // Start time loop
  ans_SDR += Type(0.5)*(initN(time)*initN(time))/(SDR*SDR)+0*log(SDR*SDR);
}
