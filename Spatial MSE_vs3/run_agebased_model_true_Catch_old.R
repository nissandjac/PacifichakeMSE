...
run.agebased.true.catch <- function(df, seed = 100){
  ...
  Fseason.save <- array(0,dim = c( nyear+1, nseason,nspace))
   ...
   # }
    ...
    for (season in 1:nseason){
      for (space in 1:nspace){
        ...
        if(season == 1){
          ...
          Bcatch <- rep(0,nage)
          Bcatch[2:(nage-1)] <- N.tmp[1:(nage-2)]*exp(-Mseason[1:(nage-2)]*pope.mul)
          Bcatch[nage] <- (N.tmp[nage-1]*exp(-Mseason[nage-1]*pope.mul)+N.tmp[nage]*exp(-Mseason[nage]*pope.mul))
          
          B.tmp <-  sum(Bcatch*w_catch*Fsel) # Get biomass from previous year
          
          Nmid[2:nage] <- N.tmp[1:(nage-1)] 
          Nmid[1] <- 0
          }else{
          N.tmp <- N.save.age[,idx,space,season-1]
            
          Bcatch <- N.tmp*exp(-Mseason*pope.mul)
          B.tmp <-  sum(Bcatch*w_catch*Fsel) # Get biomass from previous season
          Nmid <- N.tmp
        }
        ...
        if(E.temp > 0){
          ...
          for(i in 1:4){
            ...
            Alpha <- (1-exp(-Z))
            # 
            Ncatch[2:(nage-1)] <- N.tmp[1:(nage-2)]*exp(-Z[1:(nage-2)])
            Ncatch[nage] <- (N.tmp[nage-1]*exp(-Z[nage-1])+N.tmp[nage]*exp(-Z[nage]))
            # Ncatch <- N.tmp
            
            Ctmp <- sum((Fnew/Z)*(Ncatch*w_catch*Fsel)*Alpha)
            ...
            temp <- sum(Bcatch*w_catch*Fsel*Alpha)
          ...
          Z <- Fnew*Fsel+Mseason
          Fseason.save[idx,season,space] <- Fnew
          Fseason <- Fnew*Fsel
        }else{
          Fseason <- 0
        }
        ...
        Nend <- N.save.age[,idx,space,season]
        print(Ncatch/Nend)
        # 
        Catch.save.age[, idx,space, season] <- (Fseason/(Z))*(1-exp(-(Z)))*Nend*w_catch
        CatchN.save.age[, idx,space, season] <- (Fseason/(Z))*(1-exp(-(Z)))*Nend
        ...
        Ntot <- N.save.age[,idx,space,season]
        age_comps_OM[,idx,space,season] <- N.save.age[, idx,space,season]/sum(N.save.age[, idx,space,season])
        
        Nend <- N.save.age[,idx,space,season]
        
        SSB.all[idx,season,space]<- sum(Nend*Mat.sel, na.rm = T)*0.5
        Catch.save.age[, idx,space, season] <- (Fseason/(Z))*(1-exp(-(Z)))*Nend*w_catch
        CatchN.save.age[, idx,space, season] <- (Fseason/(Z))*(1-exp(-(Z)))*Nend
        }
    ...
    for (space in 1:nspace){
      if(df$surveyseason == 1){
        ...
        surveyseason <- df$surveyseason-1
      }
      
    survey.true[space,idx] <- sum(N.save.age[,idx,space, nseason]*surv.sel*q*w_surv)
    ...
    if(df$move == FALSE){
      ...
      Nsurv <- (N.save.age[,idx,space, nseason])
    }
    ...
    if (df$flag_survey[yr] == 1){
     ...
     if(df$flag_survey[yr] == 1){
      ...
      for(space in 1:nspace){
      Ntot.year <- N.save.age[,idx,space,surveyseason]
