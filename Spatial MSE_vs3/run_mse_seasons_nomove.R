...
plot(df$years,sim.data$SSB, type = 'l', ylab = 'SSB')
...
for (time in 1:simyears){ #
  ...
  yl <- ylimits(SSB,sim.data$SSB)
  plot(year.future[1:year],SSB, ylim = yl)
  lines(year.future[1:(year)],sim.data$SSB) # One model takes SSB in the beginning of the year the other in the end
  #rep<-sdreport(obj)
