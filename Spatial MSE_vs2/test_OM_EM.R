...
source('Check_Identifiable_vs3.R') # see if hessian is positive definite 
...
#@df2 <- load_data()
parms.true <- df$parms
parms.true$F0 <- rowSums(sim.data$Fsave, na.rm = TRUE)
...
lines(df$years,rowSums(colSums(sim.data$CatchN.save.age)))
