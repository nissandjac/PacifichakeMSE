...
source('run_agebased_model_true_recruittment.R')
source('ylimits.R')
source('SSB0calc.R')
source('getSelec.R')
...
for (time in 1:simyears){
  ...
  #
  Fnew <- 0.3 # Just fish at 0.3 into the future
...
png(file = 'OM_output_recruitment.png',width = 800, height = 400)
...
plot(df$years,am.US, type = 'l', ylab = 'average age', xlab = 'year', col = 'blue', ylim = c(0,6.5))
...
p1 <- ggplot(us.states,aes(x=long,y=lat,group=group))+
  ...
  coord_map()+theme_bw()+scale_x_continuous(limit = c(-143,-121),expand=c(0,0), name = '')+
