...
SSBminmax <- read.csv('minmaxSSB.csv')
moveparmin <- matrix(NA, 2,3)
moveparmin[1,] <- movemax.parms[c(5,1,2)]
moveparmin[2,] <- movefifty.parms[c(1,5,1)]


df <- load_data_seasons_move(move = TRUE, movemax.parms[2],movefifty.parms[1])
...
for(j in 1:3){
  for (i in 1:nruns){
    tmp <- try(run_multiple_MSEs(
      simyears = 30, seeds[i], 
                                 moveparms = moveparmin[,j]),
               silent = FALSE)
...
# # # 
save(ls.save,file = 'MSErun_movement.Rdata')
###  Plot the true SSB ###
#load('MSErun.Rdata')
