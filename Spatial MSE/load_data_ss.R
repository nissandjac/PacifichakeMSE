...
load_data_ss <- function(mod){
  ...
  btmp <- mod$recruit[, c("Yr", "dev", "biasadjuster")]
  
  b <- as.numeric(btmp$biasadjuster[btmp$Yr >1965 & btmp$Yr <2018])
  ...
  # Get the catchability parameter 
  ...
  df <-list(      #### Parameters #####
                  ...
                  logSDF = log(0.1),
                  # Catch
                  Catchobs = catches.obs, # Convert to kg
