...
run.agebased.true_seasons <- function(df, seed = 100){
  ...
  nyear <-df$tEnd/df$nseason # Add one year for the initial conditions
  year <- df$years
  nseason <- df$nseason
  tEnd <- nyear*nseason
  # True values 
  ...
  SSB <- matrix(NA,tEnd+1, nspace)
  Biomass.save <- matrix(NA,tEnd+1, nspace)
  Catch <- matrix(NA,tEnd+1)
  Catch.age <- matrix(NA,nage,tEnd+1)
  R.save <- matrix(NA,tEnd+1, nspace)
  
  N.save.age <- array(NA,dim = c(nage,tEnd+1, nspace))
  ...
  for (space in 1:nspace){
    N.save.age[,1,space] <- Ninit*move.init[space,] # Just to initialize 
    SSB[1,space] <- sum(N.save.age[,1,space]*Mat.sel, na.rm = T)*0.5
  }
  ...
  for (yr in 1:(nyear)){ # Loop over years add one year for initial distribution
    ...
    for (space in 1:nspace){
      ...
      N.save.age[2:(nage-1),idx,space] <- N.save.age[1:(nage-2), idx-1,space]*exp(-Z[1:(nage-2)])*(1-movemat[space,1:(nage-2)])-
        N.save.age[1:(nage-2), idx-1,space]*exp(-Z[1:(nage-2)])*(movemat[space,1:(nage-2)])+ # Remove the ones that leave``
        N.save.age[1:(nage-2), idx-1,spaceidx]*exp(-Z[1:(nage-2)])*(movemat[spaceidx,1:(nage-2)])# add the ones come to the surrounding areas
      
      N.save.age[nage, idx,space] <-  (N.save.age[nage-1, idx-1,space]*exp(-Z[nage-1])+N.save.age[nage, idx-1,space]*exp(-Z[nage]))*(1-movemat[space,nage])-
        (N.save.age[nage-1, idx-1,space]*exp(-Z[nage-1])+N.save.age[nage, idx-1,space]*exp(-Z[nage]))*(movemat[spaceidx,nage])+# Plus group
        (N.save.age[nage-1, idx-1,spaceidx]*exp(-Z[nage-1])+N.save.age[nage, idx-1,spaceidx]*exp(-Z[nage]))*(movemat[spaceidx,nage])# Plus group
    ...
    if (df$flag_survey[yr] == 1){
      ...
      surv <- sum(rowSums(N.save.age[,idx,])*surv.sel*q*w_surv)*err # If the xtra factor is not included the mean is > 1
      survey[yr] <- surv
    }else{
      survey[yr] <- 1
    }
    Ntot.year <- rowSums(N.save.age[,idx,])
  ...
  Nsave <- apply(N.save.age,2,rowSums)
  ...
  df.out   <- list(N.save = Nsave[,2:(nyear+1)], SSB = SSB[2:(nyear+1)], 
                   ...
                   survey = survey, age_comps_catch = age_comps_catch, age_comps_surv= age_comps_surv, Nout = N.save.age[,nyear+1,])
