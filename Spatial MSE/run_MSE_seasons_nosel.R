...
library(TMB)
compile("runHakeassessment_nosel.cpp")

seedz <- 100
set.seed(seedz)
dyn.load(dynlib("runHakeassessment_nosel"))
...
for (time in 1:simyears){
  ...
  parms <- list( # Just start all the simluations with the same initial conditions 
    ...
    psel_fish = exp(c(2.486490, 0.928255,0.392144,0.214365,0.475473)),
    psel_surv = exp(c(0.568618,-0.216172,0.305286 ,0.373829)),
    initN = initN,
    Rin = Rdev
  )
  ...
  obj <-MakeADFun(df.new,parms,DLL="runHakeassessment_nosel", silent = FALSE, random = 'Rin') # Run the assessment 
