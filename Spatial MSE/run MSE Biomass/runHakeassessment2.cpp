      ...
      ans_catchcomp +=lgamma(beta)-lgamma(ss_catch(time)+beta);
        for(int i=0;i<age_maxage;i++){ // Loop over other ages (first one is empty for survey)
          if(age_catch(i,time)> 0){ // Ignore zero entries
            ans_catchcomp +=  lgamma(ss_catch(time)*age_catch(i,time)+
              beta*age_catch_est(i,time))-
                lgamma(beta*age_catch_est(i,time));//lgamma(ss_catch(time)+1)-lgamma(ss_catch(time)*age_catch(i,time)+1)+
        }
      }
    }
}
