...
for(int time=0;time<tEnd;time++){ // Start time loop
  ...
  Type Bret = 0;
  vector<Type> Nmid(nage);

  for(int j=0;j<(nage);j++){ // Total biomass available to the fishery
  Nmid(j) = Ntmp(j) *exp(-Myear(j)*Type(0.5));
  Bret += Nmid(j)*catchselec(j)*wage_mid(j,time); // Total biomass available to the fishery
  }
  REPORT(Nmid)
  // //
  Type temp = Catchobs(time)/(Bret+Type(0.1)*Catchobs(time));
  Type joiner = 1/(1+exp(30*(temp-0.95)));
  Type temp_s = joiner*temp+0.95*(1-joiner);
  // Type Fin = 0.;
  // Fin = F0(time);
  Type Fin = -log(1-temp_s);
  int F_Tune = 4;
  Type join1 = 0;

  vector<Type> Z_rate=Myear+Fin*catchselec; // Total mortality
  vector<Type> Zrate2=(1.-exp(-Z_rate))/Z_rate;

  for(int tune_F=0;tune_F<F_Tune;tune_F++){ // 4 iterations



     if(tune_F<(F_Tune-1)){
       //  now calc adjustment to Z based on changes to be made to Hrate
       Type interim_tot_catch = 0;

       for(int i=0;i<(nage);i++){ // Loop over other ages
        interim_tot_catch += Fin*Ntmp(i)*catchselec(i)*wage_catch(i,time)*Zrate2(i);
       }
//
//   Type Z_adjuster = Catchobs(time)/(interim_tot_catch+0.0001);
//
...
//      Type vbio = 0;
//      for(int i=0;i<(nage);i++){ // Loop over other ages
//      vbio +=Ntmp(i)*catchselec(i)*Zrate2(i);
//     }
//
//      Type temp2=Catchobs(time)/(vbio+0.0001);  //  prototype new F
//      join1= 1./(1.+exp(30.*(temp2-0.95*1)));
//      Fin=join1*temp2 + (1.-join1)*1;  //  new F value for this fleet
//
//    }else{
//       for(int i=0;i<(nage);i++){ // Loop over other ages
//        CatchAge(i,time)=Fin*Ntmp(i)*Zrate2(i)*wage_catch(i,time);
//        CatchNAge(i,time)=Fin*Ntmp(i)*Zrate2(i);
...
// REPORT(Zrate2)
  ...
  for(int i=0;i<nage;i++){ // Loop over other ages
  CatchAge(i,time)= (Freal(i)/(Z(i)))*(1-exp(-Z(i)))*Ntmp(i)*wage_catch(i,time);// Calculate the catch in kg
  CatchNAge(i,time)= (Freal(i)/(Z(i)))*(1-exp(-Z(i)))*Ntmp(i);// Calculate the catch in kg
  Catch(time) += CatchAge(i,time);
  CatchN(time) += CatchNAge(i,time);
  }
    ...
    //
    for(int i=1;i<(nage-1);i++){ // Loop over other ages
    N(i,time) = Ntmp(i-1)*exp(-Myear(i-1))-CatchNAge(i-1,time);
    }
    // Plus group
    N(nage-1, time) = Ntmp(nage-2)*exp(-Myear(nage-2))-CatchNAge(nage-2,time)+Ntmp(nage-1)*exp(-M(nage-1))-CatchNAge(nage-1,time);
    //
  //  Catch(time) = 0;
