...
Ninit(0) = (4*h*Rinit*SSBinit/(SSBzero*(1-h)+ SSBinit*(5*h-1)))*exp(-0.5*b*SDR*SDR+logR(0));
...
//     // Biomass from survey (add 0.5 to M and F later bc it's in the middle of the season)
    ...
    // Recruitment
    R(time) = (4*h*Rinit*SSB(time-1)/(SSBzero*(1-h)+ SSB(time-1)*(5*h-1)))*exp(-0.5*b*SDR*SDR+logR(time));
...
for(int time=1;time<tEnd;time++){ // Survey biomass
      ...
      if(survey_x(time) == 2){
        ans_survey += -dnorm(log(Biomass(time)), log(survey(time)), SDsurv, TRUE);
    }
  }
