...
//  PARAMETER(a);
  ...
  Type h = exp(logh)/(1+exp(logh)); // logit transformation
  ...
  // Type psel_fish = exp(lpsel_fish);
  // Type psel_surv = exp(lpsel_surv);
...
Type ans = ans_SDR+ans_catch+ans_survey-ans_survcomp-ans_catchcomp;
//
REPORT(ans)
REPORT(ans_SDR)
REPORT(ans_survey)
