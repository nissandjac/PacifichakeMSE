...
run.agebased.true.catch.move <- function(df, seeds = 100){
  # Constant parameters
    ...
    for (season in 1:nseason){
      for (space in 1:nspace){
        ...
        if(season <nseason){
          ...
          N.save.age[,yr,space,season] <- N.save.age[,yr,space,season]-
            N.save.age[, yr,space,season]*(movemat[space,,season,yr])+ # Remove the ones that leave
            N.save.age[, yr,spaceidx,season]*(movemat[spaceidx,,season,yr])# add the ones come to the surrounding areas
          ...
          N.save.age[,yr,space,1] <- N.save.age[,yr,space,season]-
            N.save.age[, yr,space,season]*(movemat[space,,season,yr])+ # Remove the ones that leave
            N.save.age[, yr,spaceidx,season]*(movemat[spaceidx,,season,yr])# add the ones come to the surrounding areas
          ...
          N.save.age[nage,yr+1,space,1] <- Nsurvive.plus
