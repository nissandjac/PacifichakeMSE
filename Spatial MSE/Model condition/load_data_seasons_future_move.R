## Load the hake data
# year and age input 
load_data_seasons_future_move <- function(yr.future, move = TRUE, movemaxinit = 0.5, movefiftyinit = 5){
  ...
  if(max(years) > 2017){
    ...
    idx.future <- length(1966:2017)+seq(2,yr.future, by = 2) # Years where survey occurs 
    ...
    df$flag_catch[df$years >2016] <- 1 # Catch comps in all future years
     
    Rdevs <- rnorm(yr.future, mean = 0, sd = exp(df$logSDR))
